...
def read_protected_excel(file_path, password, sheet_name=None):
    with open(file_path, 'rb') as file:
        try:
            ...
            if file_decryptor.is_encrypted():
                ...
                file.seek(0)
                dfs = pd.read_excel(file, sheet_name=sheet_name, engine='openpyxl')
...
def extract_db():
    if request.method == 'POST':
        ...
        if connection.is_connected():
            ...
            if user:
                for row in user:
                    temp = {'State Name':'','FPS_District': row[1], 'FPS_Name': row[2], 'FPS_ID': row[3], 'Type': row[4], 'FPS_Lat': row[5], 'FPS_Long': row[6], 'Minimum': row[8], 'Rice_Inventory': row[9], 'Milling_Process': row[10], 'MIN_FLOW': row[11] }
                    fps_data.append(temp)
                    #print(fps_data)
            ...
            with open(json_file_path, 'w') as json_file:
                json.dump(all_data, json_file, indent=2, default=default_converter)
        else:
            import decimal
            def default_converter(o):
                if isinstance(o, decimal.Decimal):
        ...
        fps = fps.loc[:,["State Name","FPS_District",'FPS_Name',"FPS_ID","Type",'FPS_Lat',"FPS_Long","Minimum","Rice_Inventory","Milling_Process","MIN_FLOW"]]
    ...
    if request.method == 'POST':
        try:
            ...
            wh = wh.loc[:, [
                ...
                "SW_Long", "Storage", "Demand"
            ]]

            fci = fci.loc[:, [
                "WH_District", "WH_Name", "WH_ID", "Type of WH",
                "WH_Lat", "WH_Long", "Inventory1"
            ]]
            ...
            return jsonify(convert_decimal(response))
    ...
    try:
        ...
        for i in range(len(FCI["WH_District"])):
            ...
            if District_Name not in District_Capacity:
                District_Capacity[District_Name] = float(FCI["Paddy_Procurement"][i])
            else:
                District_Capacity[District_Name] += float(FCI["Paddy_Procurement"][i])
        ...
        for i in range(len(FPS["FPS_District"])):
            ...
            if District_Name_FPS not in District_Demand:
                District_Demand[District_Name_FPS] = float(FPS["Milling_Process"][i])
            else:
                District_Demand[District_Name_FPS] += float(FPS["Milling_Process"][i])
...
def graph_dataleg1():
    try:
        ...
        for i in range(len(fci["WH_District"])):
            District_Name = fci["WH_District"][i]
            if District_Name not in District_Supply:
                District_Supply[District_Name] = float(fci["Inventory1"][i])
            else:
                District_Supply[District_Name] += float(fci["Inventory1"][i])
        ...
        for i in range(len(wh["SW_District"])):
            District_Name_FPS = wh["SW_District"][i]
            if District_Name_FPS not in District_Capacity:
                District_Capacity[District_Name_FPS] = float(wh["Storage"][i])
            else:
                District_Capacity[District_Name_FPS] += float(wh["Storage"][i])
        ...
        District_Name = [
            i for i in District_Capacity
            if i in District_Supply and District_Supply[i] > District_Capacity[i]
        ]
...
def save_to_database(month, year, day):
    ...
    if connection.is_connected():
        ...
        connection.commit()
        create_pc_query = ("CREATE TABLE " + pc_table + " (uniqueid VARCHAR(100) NOT NULL, district VARCHAR(100) NOT NULL, name VARCHAR(100) NOT NULL, id VARCHAR(100) NOT NULL, latitude VARCHAR(100) NOT NULL, longitude VARCHAR(100) NOT NULL, quantity_arrival VARCHAR(100) NOT NULL, active VARCHAR(10) NOT NULL DEFAULT '1')")
        ...
        create_mill_query = ("CREATE TABLE " + mill_table + " (uniqueid VARCHAR(100) NOT NULL, district VARCHAR(100) NOT NULL, name VARCHAR(100) NOT NULL, id VARCHAR(100) NOT NULL, type VARCHAR(100) NOT NULL, latitude VARCHAR(100) NOT NULL, longitude VARCHAR(100) NOT NULL, active VARCHAR(10) NOT NULL DEFAULT '1', incoming_min_paddy VARCHAR(100) NOT NULL, total_rice_inventory VARCHAR(100) NOT NULL, milling_capacity VARCHAR(100) NOT NULL, minimum_outgoing_rice VARCHAR(100) NOT NULL)")
...
def save_to_database_leg1(month, year, day):
    ...
    if connection.is_connected():
        ...
        if(check_year_month_exists_leg1(connection, month, year, day)):
            ...
            # _mill_replicatable = "mill_replica_leg1_" + str(existingid)
            cursor.execute(sql)
        else:
        ...
        # create_replica_query = ("CREATE TABLE " + mill_replica_table + " (uniqueid VARCHAR(100) NOT NULL, district VARCHAR(100) NOT NULL, to_district VARCHAR(100) NOT NULL, name VARCHAR(100) NOT NULL, id VARCHAR(100) NOT NULL, type VARCHAR(100) NOT NULL, latitude VARCHAR(100) NOT NULL, longitude VARCHAR(100) NOT NULL, active VARCHAR(10) NOT NULL DEFAULT '1', incoming_min_paddy VARCHAR(100) NOT NULL, total_rice_inventory VARCHAR(100) NOT NULL, milling_capacity VARCHAR(100) NOT NULL, minimum_outgoing_rice VARCHAR(100) NOT NULL)")
...
def processFile():
    ...
    if scenario_type is not None and scenario_type.lower() == "intra":
        ...
        FCI.rename(columns={
            'Paddy_Procurement': 'Storage_Capacity',
        }, inplace=True)
        ...
        districts = set(FCI['WH_District']).union(set(FPS['FPS_District']))
        for d in districts:
            total_supply = FCI.loc[FCI['WH_District'] == d, 'Storage_Capacity'].sum()
            total_demand = FPS.loc[FPS['FPS_District'] == d, 'Demand'].sum()
            ...
            if total_supply >= total_demand:
                ...
                for j in range(len(DV_Variables1)):
                    ...
                    lst = name.split("_")
                    # lst[2] = warehouse district, lst[4] = FPS district
        ...
        for i in range(len(FPS['FPS_ID'])):
            model += lpSum(Allocation1[j][i] for j in range(len(FCI['WH_ID']))) <= FPS['Demand'][i]

        for i in range(len(FPS['FPS_ID'])):
            model += lpSum(Allocation1[j][i] for j in range(len(FCI['WH_ID']))) >= FPS['Minimum'][i]

        for i in range(len(FCI['WH_ID'])):
            model += (lpSum(Allocation1[i][j] for j in range(len(FPS['FPS_ID']))) <= FCI['Storage_Capacity'][i])

        for i in range(len(FCI['WH_ID'])):
            model += (lpSum(Allocation1[i][j] for j in range(len(FPS['FPS_ID']))) >= FCI['Storage_Capacity'][i])


        model.writeLP('Backend/debug.lp')
        ...
        if status != "Optimal":
            ...
            return json.dumps(convert_decimal(data), indent=1)
...
# ----------------------------------------------------------------------------------------------------------------------------------------------
        ...
        if 'Paddy_Procurement' in FCI.columns:
            FCI.rename(columns={'Paddy_Procurement': 'Storage_Capacity'}, inplace=True)
            
        if 'Milling_Process' in FPS.columns:
            FPS.rename(columns={'Milling_Process': 'Allocation_Wheat'}, inplace=True)
            
        if 'Rice_Inventory' in FPS.columns:
            FPS.rename(columns={'Rice_Inventory': 'Allocation_Rice'}, inplace=True)
        elif 'Allocation_Rice' not in FPS.columns:
            FPS['Allocation_Rice'] = 0

        if 'FPS_Tehsil' not in FPS.columns:
            FPS['FPS_Tehsil'] = FPS.get('FPS_District', '')

        # print(FCI.columns)
        FCI['WH_District'] = FCI['WH_District'].apply(lambda x: str(x).replace(' ', ''))
        FPS['FPS_District'] = FPS['FPS_District'].apply(lambda x: str(x).replace(' ', ''))
    ...
    #    Variables for Wheat from IG TO FPS
       ...
       #model.solve(CPLEX_CMD(options=['set mip tolerances mipgap 0.03',"set emphasis memory y"]))
        ...
        df511.insert(2, 'From_State', 'Telangana')
        df511.insert(7, 'To', 'FPS')
        df511.insert(8, 'To_State', 'Telangana')
...
def processFile_leg1():
    ...
    if scenario_type == "intra":
        ...
        Total_Supply = []
        Total_Supply_Warehouse = {}
        Total_Supply = FCI['Inventory1'].sum()
        Total_Supply_Warehouse['Total_Supply_Warehouse'] = Total_Supply  # Total SUPPLY
        ...
        for i in range(len(FCI['WH_District'])):
            District_Name = FCI['WH_District'][i]
            if District_Name not in District_Capacity:
                District_Capacity[District_Name] = FCI['Inventory1'][i]
            else:
                District_Capacity[District_Name] = FCI['Inventory1'][i] + District_Capacity[District_Name]
        ...
        for i in range(len(FCI["WH_District"])):
            District_Name = FCI["WH_District"][i]
            if District_Name not in District_Capacity:
                District_Capacity[District_Name] = float(FCI["Inventory1"][i])
            else:
                District_Capacity[District_Name] += float(FCI["Inventory1"][i])
       ...
       # Calling CBC_CMB Solver
        ...
        Cost = pd.ExcelFile("Backend//Distance_Intial_L1.xlsx")
        BG_BG = pd.read_excel(Cost,sheet_name="BG_BG")
...
# ----------------------------------------------------------------------------------------------------------
        ...
        WH.rename(columns={
            ...
            'Storage': 'Capacity', }, inplace=True)
        districts = set(FCI['WH_District']).union(set(WH['SW_District']))
        for d in districts:
            total_supply = FCI.loc[FCI['WH_District'] == d, 'Inventory1'].sum()
            total_demand = WH.loc[WH['SW_District'] == d, 'Annual_Demand'].sum()
            ...
            if total_supply >= total_demand:
                ...
                for j in range(len(DV_Variables1)):
                    ...
                    if lst[2] == d and lst[4] != d:
                        model += DV_Variables1[j] == 0
        TotalSupply=FCI['Inventory1'].sum()
        TotalDemand=WH['Annual_Demand'] .sum()
        TotalCapacity=WH['Capacity'] .sum()
        ...
        if TotalSupply <= TotalDemand :  
            ...
            for i in range(len(FCI['WH_ID'])):      # for each mill
                model += lpSum(Allocation1[i][j] 
                        for j in range(len(WH['SW_ID']))) >= MIN_FLOW
        ...
        df4 = pd.read_excel('Backend//Tagging_Sheet_Pre_leg21.xlsx')
        ...
        df4['WH_ID'] = pd.to_numeric(df4['WH_ID'], errors='coerce')
        FCI['WH_ID'] = pd.to_numeric(FCI['WH_ID'], errors='coerce')
        df4['SW_ID'] = pd.to_numeric(df4['SW_ID'], errors='coerce')
        WH['SW_ID'] = pd.to_numeric(WH['SW_ID'], errors='coerce')
